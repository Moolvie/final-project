...
//////// START ADD FUNCTIONS ////////
	...
	{			
		function AddPlaylist($db, $playlistName, $CustomerID)
		{
	...
	// Adds a song to a playlist within the database
	if(!function_exists('AddPlaylistSong'))
	{
		function AddPlaylistSongs($query, $db, $playlistID)
		{
			// run query 
			if (!$query)
				die($db->error);
			{
				...
				{
					$db->query("INSERT IGNORE INTO playlist_songs(PlaylistID, SongID) ".
					"VALUES('$playlistID', '{$row['SongID']}')");
				}
